 ...
 */
export function resolveModel(modelStr: string | undefined, defaultProvider?: string): ResolvedModel {
	if (!modelStr) {
	...
	for (let i = messages.length - 1; i >= 0; i--) {
		...
		if (msg?.role === "assistant" && Array.isArray(msg.content)) {
			for (const part of msg.content) {
				if (typeof part === "object" && part !== null && "type" in part && part.type === "text" && "text" in part && part.text) {
...
export function getToolCalls(messages: Message[]): Array<{ name: string; args: Record<string, unknown> }> {
		...
		if (msg?.role === "assistant" && Array.isArray(msg.content)) {
			for (const part of msg.content) {
				if (
					...
					calls.push({ name: part.name as string, args: (part.arguments as Record<string, unknown>) || {} });
				}
			}
		}
	}
...
export async function runPiJsonAgentWithUI(
	...
	const result = await ctx.ui.custom<RunAgentResult>((tui, theme, _kb, done) => {
		...
		const buildHeaderMeta = () => {
			...
			if (currentResult.durationMs > 0) parts.push(`${(currentResult.durationMs / 1000).toFixed(1)}s`);
			return parts.join(" ");
		};
		...
		runPiJsonAgent({
			...
			onUpdate: (partial) => {
				...
				statusText = getFinalAssistantText(partial.messages).split("\n").slice(0, 3).join("\n") || "(working...)";
				updateDisplay();
			},
		}).then((final) => {
			currentResult = final;
