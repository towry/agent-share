	...
	for (const msg of messages) {
		...
		for (const part of msg.content) {
			if (part && typeof part === "object" && "type" in part) {
				const typed = part as { type?: string; text?: string; name?: string; arguments?: Record<string, any> };
				if (typed.type === "text" && typeof typed.text === "string") {
					items.push({ type: "text", text: typed.text });
				} else if (typed.type === "toolCall" && typeof typed.name === "string") {
					items.push({ type: "toolCall", name: typed.name, args: typed.arguments ?? {} });
				}
			}
		}
	}
...
async function runSingleAgent(
	...
	const emitUpdate = () => {
		if (onUpdate) {
			onUpdate({
				content: [{ type: "text", text: getFinalAssistantText(currentResult.messages) || "(running...)" }],
				details: makeDetails([currentResult]),
			});
		}
	};

	// Parse model field: "provider:model" or just "model"
	...
	const result = await runPiJsonAgent({
		...
		onUpdate: (partial) => {
			...
			currentResult.usage = {
				...
				contextTokens: partial.usage.totalTokens,
	...
	currentResult.model = result.model;
	...
	pi.registerTool<typeof SubagentParams, SubagentDetails>({
		...
		async execute(_toolCallId, params, onUpdate, ctx, signal) {
			...
			if (params.chain && params.chain.length > 0) {
				...
				for (let i = 0; i < params.chain.length; i++) {
					...
					if (isError) {
						const errorMsg =
							result.errorMessage || result.stderr || getFinalAssistantText(result.messages) || "(no output)";
						return {
					...
					previousOutput = getFinalAssistantText(result.messages);
				}
				return {
					content: [{ type: "text", text: results.length > 0 ? getFinalAssistantText(results[results.length - 1]!.messages) || "(no output)" : "(no output)" }],
					details: makeDetails("chain")(results),
				};
			}
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				const summaries = results.map((r) => {
					const output = getFinalAssistantText(r.messages);
			...
			if (params.agent && params.task) {
				...
				return {
					content: [{ type: "text", text: getFinalAssistantText(result.messages) || "(no output)" }],
					details: makeDetails("single")([result]),
				};
			}
		...
		renderResult(result, { expanded }, theme) {
			...
			if (details.mode === "single" && details.results.length === 1) {
				...
				const finalOutput = getFinalAssistantText(r.messages);
