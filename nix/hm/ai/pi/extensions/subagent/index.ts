...
async function runSingleAgent(
  ...
  const emitUpdate = () => {
    if (onUpdate) {
      onUpdate({
        content: [
          { type: "text", text: getFinalAssistantText(currentResult.messages) || "(running...)" },
        ],
  ...
  // Validate model format: must use "provider:model" syntax
  if (agent.model && !agent.model.includes(":")) {
    return {
      ...
      exitCode: 1,
      messages: [],
      stderr: `Model "${agent.model}" must use "provider:model" format (e.g., "ant-sdk:bender-muffin").`,
      usage: {
        input: 0,
        output: 0,
        cacheRead: 0,
      ...
      errorMessage: `Invalid model format: "${agent.model}". Use "provider:model" syntax.`,
      step,
    };
  }
  ...
  pi.registerTool<typeof SubagentParams, SubagentDetails>({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
          ...
          content: [
            {
              ...
              text:
                results.length > 0
                  ? getFinalAssistantText(results[results.length - 1]!.messages) || "(no output)"
                  : "(no output)",
            },
          ],
          details: makeDetails("chain")(results),
      ...
      if (params.agent && params.task) {
        ...
        return {
          content: [
            { type: "text", text: getFinalAssistantText(result.messages) || "(no output)" },
          ],
          details: makeDetails("single")([result]),
        };
      }
