...
Before spawning investigators, list the MCP servers and tools this session actually has. Use the harness tool/MCP map when present.
...
- Role: general-purpose subagent, `fast-code`, full tool access
- Do not use a read-only mode that strips MCP access. That disables MCP-backed investigators. Investigators still must not write files. That's a posture, not a sandbox.
...
- Role: general-purpose subagent, `judgment`, full tool access
- The synthesizer spot-verifies citations and may need MCP access. A read-only mode that strips MCPs defeats that.
