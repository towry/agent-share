...
- Role: general-purpose subagent, `fast-code`, read-only

Each explorer gets the same base prompt from `references/explorer-prompt.md` plus a specific exploration angle naming its slice. Each explorer should:
- Start broad: search relevant directories, then search for key types/interfaces/class names
...
- Role: general-purpose subagent, `judgment`, read-only

The agent does its own exploration (directory search, symbol search, file reads) and writes the explanation directly. Read `references/explainer-prompt.md` for the communication style and output format. Same structure, just no explorer findings as input.
...
After the explanation is complete, spawn one architectural critic per entry in the `how-critics` list from setup-pstack `references/model-roles.md` (defaults `judgment`, `instruction-following`, `fast-code`, `extra-voice`), all in a single message.

For each critic:
- Role: general-purpose subagent, read-only
- Model: one entry from `how-critics`. These are minimum reasoning levels. Escalate when the architecture warrants deeper analysis.
